...
async def create_season(
    ...
    existing = (
        await session.execute(
            select(Season).where(Season.show_id == show_id, Season.season_number == payload.season_number)
        )
    ).scalar_one_or_none()
    if existing:
        raise ApiError(
            ...
            [
                {
                    "code": "SEASON_NUMBER_UNIQUE",
                    "field": "season_number",
                    "message": f"Season {payload.season_number} already exists.",
                    "hint": None,
                    "resource": {"type": "season", "id": existing.id},
                }
            ],
        )
...
async def update_season(
    ...
    await session.commit()
    cnt = (
        await session.execute(select(func.count()).select_from(Episode).where(Episode.season_id == season.id))
    ).scalar_one()
...
async def delete_season(
    ...
    published = (
        await session.execute(
            select(func.count())
            .select_from(Episode)
            .where(Episode.season_id == season_id, Episode.status == "published")
        )
    ).scalar_one()
    if published:
        raise ApiError(
            409,
            "conflict",
            f"Season {season.season_number} still holds {published} published episode(s). "
            "Unpublish them before deleting.",
        )
